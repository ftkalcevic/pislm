...
#define SAMPLE_HZ				10
#define SAMPLE_BUFFER_SIZE		(SAMPLE_RATE * (SAMPLE_FORMAT==SND_PCM_FORMAT_S32_LE ? 4 : SAMPLE_FORMAT==SND_PCM_FORMAT_S24_LE ? 3 : 2) * (SAMPLE_MODE_STEREO==1?2:1) / SAMPLE_HZ)
#define FRAMES_PER_PERIOD		(SAMPLE_RATE / SAMPLE_HZ)
...
int CAudio::init()
{
	...
	chunk_size = -1;
	rhwparams.format = SAMPLE_FORMAT;
	rhwparams.rate = SAMPLE_RATE;
	rhwparams.channels = SAMPLE_MODE_STEREO ? 2 : 1;
	int open_mode = 0;
	int nonblock = 0;
...
void CAudio::set_params( int frames_per_period )	
{
...
//	buffer_frames = m_audioBuffer.size() / hwparams.channels;
...
//	}
//	else {
	snd_pcm_uframes_t buffer_bytes = frames_per_period * hwparams.channels * 4 /*bytes per sample*/; 		// ? times the number of periods?
...
//	}
	...
	m_audioBuffer.resize(chunk_bytes);
...
void CAudio::capture()
{
	...
	set_params(FRAMES_PER_PERIOD);

	/* get number of bytes to capture */
	count = m_audioBuffer.size();
	...
	do {
		...
		while (rest > 0 && capture_stop == 0) {
			...
			if (pcm_read(m_audioBuffer.buffer(buf&1), f) != f)
				break;
...
//			}
		...
		// Signal buf&1
		emit BufferUpdate(m_audioBuffer.buffer(buf & 1), m_audioBuffer.size());
		buf++;
